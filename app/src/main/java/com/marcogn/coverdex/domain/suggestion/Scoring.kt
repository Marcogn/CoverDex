...
/**
 * Composite score for [candidate] joining a team alongside [otherMembers], measured against
 * [currentTeamCoverage] (the real team's union coverage — kept as its own parameter rather than
 * re-derived from [otherMembers], since replacement mode compares against the *actual* team, not
 * the hypothetical one with a member removed). Ports `suggestionEngine.ts`'s
 * `computeCompositeScore` verbatim, [candidate] and [otherMembers] evaluated by types only
 * (`offensiveCoverageForMember(chart, member, useMoves = false)`), never by moves — see
...
fun computeCompositeScore(
    ...
    for (w in candWeaknesses) {
        val membersWithSameWeakness = otherWeaknessMap[w]
